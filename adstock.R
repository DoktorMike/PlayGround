...
## Functions
adStock <- function(x, lambda = 0.5) stats::filter(x, filter = lambda, method = "rec")
fisk <- function(x, a, b) 1 / (1 + (x / a)^(-b))

# Media parameters
lambda <- c(tv = 0.8, radio = 0.2, common = 0.7)
scale <- c(tv = 10, radio = 6, common = 20)
shape <- c(tv = 2, radio = 2, common = 2)
...
# Generate data
mydf <- data.frame(
    date = seq(Sys.Date(), by = "day", length.out = numEpochs),
...
resdf <- mydf %>%
    ...
    tibble::tibble()
ggplot(
    gather(resdf, method, value, -date),
    aes(y = value, x = date, fill = method)
) +
    geom_bar(stat = "identity") +
    facet_grid(method ~ .) +
...
p1 <- ggplot(
    ...
    ggtitle("Human")
p2 <- ggplot(
    gather(fakedf, method, value, -date),
    aes(y = value, x = date, fill = method)
) +
    geom_bar(stat = "identity") +
    facet_grid(method ~ .) +
...
ggplot(tmpdf, aes(y = Value, x = date, fill = Media)) +
    geom_bar(stat = "identity") +
    facet_grid(Campaign ~ .) +
    theme_minimal()
